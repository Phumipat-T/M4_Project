function addSearchTable(movie) {
    ...
    cell.addEventListener('dblclick', function () {
        let confirmAdd = confirm(`Confirm to add "${movie.title}" to your favorites or not!?`)
        if (confirmAdd) {
            addToDataBase(movie)
        }
    })
...
function addToDataBase(movie) {
    ...
    fetch(`https://se104-project-backend.du.r.appspot.com/movies `, {
        ...
        alert('Success!, this movie is add to your favorites.')
    }).catch(error => {
...
function addfavtotable(movie) {
    ...
    detailbutton.classList.add("btn", "btn-success", "btn-outline-light")
    ...
    delbutton.classList.add("btn", "btn-danger", "btn-outline-dark")
    delbutton.style.margin = "10px";
    delbutton.innerHTML = "Delete"
    delbutton.addEventListener('click', function () {
        let confirmDel = confirm(`Delete! "${movie.title}" from your favorites ?`)
        if (confirmDel) {
            delid(movie.id)
        }
    })
...
function delid(id) {
    fetch(`https://se104-project-backend.du.r.appspot.com/movie?id=632110350&&movieId=${id}`, {
        ...
        alert(`Confirm to delete ${data.title} ?`)
        getfavorite()
    }).catch(error => {
        alert('Sorry, Please try again.')
    })
...
function showdet(movie) {
    ...
    button.classList.add("btn", "btn-success", "btn-outline-light")
